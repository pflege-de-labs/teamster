...
* SQL lives in `internal/store/queries/<dialect>/`, and the Go that runs it is generated by sqlc
  into `internal/store/sqlitedb`. Edit the `.sql`, never the generated Go, and run `make generate`.
  A stale regeneration compiles and runs the old statement, so CI regenerates and diffs.
